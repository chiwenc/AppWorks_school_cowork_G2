...
def clean_data(last_time, current_time):
    ...
    rows = db.tracking_raw_realtime.find(
        {
            "created_at": {
                "$gt": last_time.strftime("%Y-%m-%d %H:%M:%S"),
    ...
    for row in rows:
        ...
        cursor.execute(
            insert_sql,
            (obj["cid"], obj["start_time"], event, view_detail, item_id, checkout_step),
        )
    conn.commit()
...
def aggregate_data(current_time):
    ...
    cursor = conn.cursor()
    select_today_user_sql = (
        "SELECT DISTINCT(client_id) FROM tracking_realtime WHERE time >= %s"
    )
    select_before_user_sql = (
        "SELECT DISTINCT(client_id) FROM tracking_realtime WHERE time < %s"
    )
    select_all_user_sql = (
        "SELECT COUNT(DISTINCT(client_id)) AS count FROM tracking_realtime"
    )
    ...
    cursor.execute(
        ...
        (
            ...
            new_user_count,
            return_user_count,
            user_behavior["view_count"],
            user_behavior["view_item_count"],
            user_behavior["add_to_cart_count"],
            user_behavior["checkout_count"],
            all_user_count,
            active_user_count,
            new_user_count,
            return_user_count,
